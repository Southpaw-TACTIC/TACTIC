...
class TzLocalTests(unittest.TestCase):
    ...
    def test_env(self):
        tz_harare = tzlocal_olson.unix._tz_from_env(':Africa/Harare')
        ...
        tz_local = tzlocal_olson.unix._tz_from_env(':' + os.path.join(local_path, 'test_data', 'Harare'))
        ...
        self.assertRaises(pytz.UnknownTimeZoneError, tzlocal_olson.unix._tz_from_env, 'GMT+03:00')

    def test_timezone(self):
        # Most versions of Ubuntu
        local_path = os.path.split(__file__)[0]
        tz = tzlocal_olson.unix._get_localzone(_root=os.path.join(local_path, 'test_data', 'timezone'))
        self.assertEqual(tz.zone, 'Africa/Harare')

    def test_zone_setting(self):
        # A ZONE setting in /etc/sysconfig/clock, f ex CentOS
        local_path = os.path.split(__file__)[0]
        tz = tzlocal_olson.unix._get_localzone(_root=os.path.join(local_path, 'test_data', 'zone_setting'))
        self.assertEqual(tz.zone, 'Africa/Harare')

    def test_timezone_setting(self):
        # A ZONE setting in /etc/conf.d/clock, f ex Gentoo
        local_path = os.path.split(__file__)[0]
        tz = tzlocal_olson.unix._get_localzone(_root=os.path.join(local_path, 'test_data', 'timezone_setting'))
        self.assertEqual(tz.zone, 'Africa/Harare')
    ...
    def test_symlink_localtime(self):
        ...
        local_path = os.path.split(__file__)[0]
        tz = tzlocal_olson_olson.unix._get_localzone(_root=os.path.join(local_path, 'test_data', 'symlink_localtime'))
        self.assertEqual(tz.zone, 'Africa/Harare')

    def test_only_localtime(self):
        local_path = os.path.split(__file__)[0]
        tz = tzlocal_olson.unix._get_localzone(_root=os.path.join(local_path, 'test_data', 'localtime'))
...
if sys.platform == 'win32':
    ...
    import tzlocal_olson.win32
    class TzWin32Tests(unittest.TestCase):

        def test_win32(self):
            tzlocal_olson.win32.get_localzone()
