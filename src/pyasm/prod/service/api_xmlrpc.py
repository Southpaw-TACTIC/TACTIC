...
def xmlrpc_decorator(meth):
    ...
    def new(my, original_ticket, *args):
        ...
        try:
            # HACK: add in a special requirement for get_widget() because
...
class ApiXMLRPC(BaseApiXMLRPC):
    ...
    def get_plugin_dir(my, ticket, plugin_code):
        ...
        if not plugin:
            return "/plugins/%s" % plugin_code

        rel_dir = plugin.get_value("rel_dir")
        return "/plugins/%s" % rel_dir
    ...
    def get_all_paths_from_snapshot(my, ticket, snapshot_code, mode='client_repo', expand_paths=False, filename_mode='',file_types=[]):
        ...
        if file_types:
            ...
            for file_type in file_types:
                path = snapshot.get_path_by_type(file_type, mode=mode)
                if path:
                    paths.append(path)
        else:
    ...
    def create_snapshot(my, ticket, search_key, context, snapshot_type="file",
        ...
        snapshot = Snapshot.create(sobject, snapshot_type=snapshot_type, context=context, description=description, is_current=is_current, level_type=level_type, level_id=level_id, is_revision=is_revision)
