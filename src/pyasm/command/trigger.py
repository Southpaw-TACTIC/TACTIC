...
class Trigger(Command):
    ...
    def _get_triggers(cls, call_event):
        ...
        # FIXME: get_db_triggers only get triggers for this project ...
        # need to update so that triggers from other projects
        # are also executed
        if trigger_dict == None:

            # build a list of site and db of the triggers for current project
    ...
    def call_by_key(cls, key, caller, output={}, forced_mode='' ):
        event = key.get("event")
        #call_event_key = jsondumps(key)
        triggers_sobjs = cls._get_triggers(key)
    ...
    def _handle_trigger_sobjs(cls, triggers_sobjs, caller, event, output, forced_mode=''):
        ...
        for trigger_sobj in triggers_sobjs:
            ...
            try:
                ...
                elif trigger_class in ['pyasm.command.EmailTrigger2', 'pyasm.command.EmailTriggerTest']:
