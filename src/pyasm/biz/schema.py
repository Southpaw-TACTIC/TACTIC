...
<schema parent="__NONE__">
    ...
    <search_type name="sthpw/access_rule"/>
...
class Schema(SObject):
    ...
    def get_relationship(my, search_type, search_type2):
        ...
        search_type: connection through search_type, search_id columns
        ...
        connect = my.xml.get_node(xpath)
        if connect == None:
            xpath = "schema/connect[@from='%s' and @to='%s']" %(search_type2, search_type)
            connect = my.xml.get_node(xpath)

        # next look at the "*" relationships
        if connect == None:
            for from_st in [search_type, search_type2]:
                xpath = "schema/connect[@from='%s' and @to='*']" %(from_st)
                connect = my.xml.get_node(xpath)
                if connect != None:
                    break


        # if no explicit relationship is defined:
        if connect == None:
            if my.parent_schema:
                relationship = my.parent_schema.get_relationship(search_type, search_type2)
                # general is last resort so skip that till recursion ends
                if relationship and relationship != 'general':
                    return relationship
                else:
                    return my.sthpw_schema.get_relationship(search_type, search_type2)
            else:
                if my.sthpw_schema:
                    return my.sthpw_schema.get_relationship(search_type, search_type2)
                else:
        ...
        # get defined relationship if connect is found
        relationship = Xml.get_attribute(connect, "relationship")
        
        if not relationship:
            relationship = 'general'

            raise TacticException("No relationship type defined for search_types [%s] and [%s]" % (search_type, search_type2) )
    ...
    def get_relationship_attrs(my, search_type, search_type2, path=None, cache=True):
        if cache:
            key = "Schema:%s|%s|%s" % (search_type, search_type2, path)
        ...
        if connect == None:
            if my.parent_schema:
                attrs = my.parent_schema.get_relationship_attrs(search_type, search_type2, path=path, cache=False)
                
                if not attrs:
                    attrs = my.sthpw_schema.get_relationship_attrs(search_type, search_type2, path=path, cache=False)
                processed = True
            else:
                if my.sthpw_schema:
                    attrs = my.sthpw_schema.get_relationship_attrs(search_type, search_type2, path=path, cache=False)
        ...
        if relationship == 'id':
            ...
            if not to_col:
                attrs['to_col'] = 'id'
                to_col = 'id'
            #if not b_search_type_obj.column_exists(to_col):
            if not SearchType.column_exists(b_search_type, to_col):
                attrs['disabled'] = True
            #if not a_search_type_obj.column_exists(from_col):
    ...
    def get_foreign_keys(my, search_type, search_type2, path=None):
        ...
        relationship = attrs.get('relationship')
        my_is_from = attrs.get('from') == search_type
        ...
        if relationship in ['id', 'code']:
            ...
            raise TacticException("Relationship [%s] is not supported" % relationship)
    ...
    def insert_trigger(my, sobject):
        ...
        if from_type and to_type:
            ...
            # DEPECATING THIS.  This is far better done with triggers
            '''
            #
            # Handle synchronized lists
            #
            ...
            parent_instance = sobject

            # find thie child types of the instance
            child_instance_types = my.get_child_types(search_type,"synchronized")
            # we have the parent
            parent_type = parent.get_base_search_type()
            child_types = my.get_child_types(parent_type)
    
            # go through each child type of the instance
            # and find the parent type
            child_type = ""
            for child_instance_type in child_instance_types:
                child_from_type, child_to_type = my.get_types_from_instance(child_instance_type)
                for x in child_types:
                    if child_to_type == x:
                        child_type = child_to_type
                        break

            if child_type:
                children = parent.get_all_children(child_type)
                for child in children:
                    child_code = child.get_code()
                    child_key = child.get_foreign_key()
                    
                    # FIXME: hard coded asset code
                    to_key = "asset_code"
                    to_code = parent_instance.get_value(to_key)

                    from pyasm.search import SearchType
                    instance = SearchType.create(child_instance_type)
                    instance.set_value(child_key, child_code)
                    instance.set_value(to_key, to_code)
                    instance.commit()
            '''


        # DEPRECATED: this is far better done with triggers
        ''' 
        #
        # Check to see if this sobject has an instance child that is
        # synchronized
        #
        child_instance_types = my.get_child_types(search_type)
        for child_instance_type in child_instance_types:

            parent_instance_type = my.get_parent_type(child_instance_type, "synchronized")
            if not parent_instance_type:
                continue
            ...
            # get all of the instances for this parent
            parent_type = my.get_parent_type(search_type)
            parent = sobject.get_parent(parent_type)
            parent_code = parent.get_code()
            parent_key = parent.get_foreign_key()

            search = Search(parent_instance_type)
            search.add_filter(parent_key, parent_code)
