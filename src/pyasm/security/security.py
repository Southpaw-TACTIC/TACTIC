    ...
    def get_by_login(login_name, namespace=None):
        ...
        if login_name == "admin":
            ...
            if not login:
                ...
                login.set_value("first_name", "Mr.")
                login.set_value("last_name", "Admin")
...
class Security(Base):
    ...
    def login_as_guest(my):
        ...
        """
        # default to admin.  Generally batch is run as admin.
        login_name = "__guest__"

        # login must exist in the database
        my._login = SearchType.create("sthpw/login")
        my._login.set_value("code", "__guest__")
        my._login.set_value("login", "__guest__")
    ...
    def login_with_ticket(my, key, add_access_rules=True):
        ...
        #print "--- security - login_with_ticket"
        #import time
        #start = time.time()
...
class License(object):
    ...
    def get_current_users(my):
        ...
        if 'license_type' in columns:
            select.add_op('begin')
            select.add_where("\"license_type\" = 'user'")
            select.add_where("\"license_type\" is NULL")
            select.add_op('or')

        select.add_op('begin')
        select.add_where('"s_status" is NULL')
        select.add_where('"s_status" != \'retired\'')
        select.add_op('or')
        select.add_where("""\"login\" != 'admin'""")
