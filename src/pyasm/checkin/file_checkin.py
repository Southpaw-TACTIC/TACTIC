...
class FileCheckin(BaseCheckin):
    ...
    def process_checkin_type(my, sobject, process, context, filepath):
        ...
        if my.checkin_type =='auto':
            ...
            if not my.dir_naming:
                ...
                if has_code:
                    my.dir_naming = {
                    'default': "{project.code}/{search_type.table_name}/{code}/{snapshot.process}/.versions"
                    }
                else:
                    my.dir_naming = {
                    'default': "{project.code}/{search_type.table_name}/{id}/{snapshot.process}/.versions"
                    }
    ...
    def create_snapshot(my, file_objects, snapshot_xml=None):
        '''
    ...
    def commit_snapshot(my, snapshot_xml):
        ...
        my.snapshot = Snapshot.create( my.sobject, snapshot_type,
            ...
            is_synced=is_synced, version=my.version)
    ...
    def get(cls, sobject, file_paths, file_types, \
        ...
        if sobject.has_value("pipeline_code"):
            ...
            if pipeline:
                # FIXME: assumption of process == context
...
class FileAppendCheckin(FileCheckin):
    ...
    def create_snapshot(my, file_objects):
        ...
        return super(FileAppendCheckin,my).create_snapshot(file_objects,xml)
...
class FileGroupAppendCheckin(FileGroupCheckin):
    ...
    def create_snapshot(my, file_objects):
        # take the current snapshot
        xml = my.append_snapshot.get_snapshot_xml()
        return super(FileGroupAppendCheckin,my).create_snapshot(file_objects,xml)
