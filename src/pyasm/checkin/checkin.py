...
class BaseCheckin(Command):
    ...
    def execute(my):
        ...
        # create the snapshot and commit
        my.snapshot_xml = my.create_snapshot(my.file_objects)
        my.commit_snapshot(my.snapshot_xml)
    ...
    def update_metadata(my, snapshot, files, file_objects):
        ...
        handler = CheckinMetadataHandler(snapshot=snapshot, files=files, file_objects=file_objects)
        handler.execute()
    ...
    def handle_file_naming(my):
        ...
        my.snapshot.commit(triggers=False)
