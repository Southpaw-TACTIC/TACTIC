...
class WidgetConfig(Base):
    ...
    # FIXME: Leaving old implementation until we can reproduce get_options()
    # perfectly
    def get_options(my, element_name, element_child_name):
        ...
        # FIXME: special case for custom layout widget
...
class WidgetConfigView(Base):
    ...
    def get_display_widget(my, element_name, recurse=False, extra_options=None, kbd_handler=True):
        ...
        if display_handler:
            try:
                ...
                if display_handler in ['tactic.ui.panel.TableLayoutWdg','tactic.ui.panel.FastTableLayoutWdg']:
                    raise
    ...
    def get_by_search_type(search_type, view, use_cache=True, local_search=False):
        ...
        # DEPRECATED
        """
        # build a config from the schema
        from pyasm.biz import Schema
        schema = Schema.get()
        # find all of the connections for this search_type
        xml = schema.get_xml_value("schema")
        connects = xml.get_nodes("schema/connect[@from='%s']" % base_search_type)
        #connects2 = xml.get_nodes("schema/connect[@to='%s']" % search_type)
        #connects.extend(connects2)

        if connects:

            schema_view = "schema_view"
            schema_xml = []
            schema_xml.append('<config><%s>' % schema_view)
            for connect in connects:
                ...
                if not to_search_type_obj:
                    continue
                database = to_search_type_obj.get_database()
                table = to_search_type_obj.get_table()

                from_col = Xml.get_attribute(connect, 'from_col')
                to_col = Xml.get_attribute(connect, 'to_col')
                display = Xml.get_attribute(connect, 'display')
                name = Xml.get_attribute(connect, 'name')
                if not name:
                    name = table

                # if display is not defined, then choose the first column
                if not display:
                    from pyasm.search import DbContainer
                    db_resource = DbResource.get_default(database)
                    columns = DbContainer.get(db_resource).get_columns(table)
                    if len(columns) >= 2:
                        display = columns[1]
                    else:
                        display = columns[0]
                ...
                if view in ['edit','insert','edit_item', 'publish']:
                    if name in ['current_subtask_status']:
                        schema_xml.append('''
                        <element name='%s'>
                          <display class='tactic.ui.widget.PipelineSelectWdg'>
                            <empty>true</empty>
                            <pipeline>subtask_status</pipeline>
                          </display>
                        </element>
                        ''' % (name) )
                else:
                    schema_xml.append('''
                    <element name='%s'>
                      <display class='tactic.ui.table.ForeignKeyElementWdg'>
                        <column>%s</column>
                      </display>
                    </element>
                    ''' % (name, from_col) )
            schema_xml.append('</%s></config>' % schema_view)
            schema_xml = "".join(schema_xml)
            schema_config = WidgetConfig.get(schema_view, xml=schema_xml)
