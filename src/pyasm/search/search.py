...
class Search(Base):
    ...
    def add_sobject_filter(my, sobject, prefix=""):
    ...
    def add_relationship_filters(my, sobjects, op='in', path=None):
        '''adds a filter to the current search that are related to the passed
        ...
        if relationship in ['id', 'code']:
            ...
            if my_is_from:
                ...
                if not multi_stypes:
                    id_values  = [x.get_value(to_col) for x in sobjects]
                    my.add_filter("search_type", sobjects[0].get_search_type() )
                    my.add_filters("search_id", id_values, op=op )
                else:
                    if op != 'in':
                ...
                sobject_values = SObject.get_values(sobjects, "search_id", unique=True)
                sobject_values = [x for x in sobject_values if x]
                my.add_filters("id", sobject_values, op=op)
                #raise SearchException("Relationship [%s] not supported yet" % relationship)
        else:
            raise SearchException("Relationship [%s] not supported yet" % relationship)
    ...
    def add_relationship_search_filter(my, search, op='in', delay_null=False):
        ...
        if relationship in ['id', 'code']:
            ...
            if my_is_from:
                ...
                id_values = search.get_sobject_ids()
                if delay_null and not id_values:
                    return False
                my.add_op('begin')
                my.add_filter("search_type", search.get_search_type() )
                my.add_filters("search_id", id_values )
                my.add_op('and')
                ...
                if not results:
                    ...
                    ids = [x[0] for x in results if x[0]]
                    if delay_null and not ids:
                        return False
                    my.add_filters("id", ids, op=op)
    ...
    def add_keyword_filter(my, column, keywords, table=None, column_type=None):
    ...
    def do_search(my, redo=False, statement=None):
        ...
        if len(results) > 1000:
            print "sobject creation(%s): %s" % (len(results), statement)
    ...
    def get_by_code(search_type, code):
    ...
    def get_related_by_sobjects(cls, sobjects, related_type, filters=[], path=None, show_retired=False):
        ...
        for related_sobject in related_sobjects:
            ...
            elif relationship in ['search_type', 'search_code']:
                if is_from:
                    ...
                    #key = "%s&code=%s" % (search_type, search_code)
                    search_id = related_sobject.get_id()
                    key = "%s&id=%s" % (search_type, search_id)
...
class SObject(object):
    ...
    # DEPRECATED: this function conflicts with SearchType.column_exists().
    #def column_exists(my, column):
    #    #search_type = my.get_search_type_obj()
    #    #return search_type.column_exists(column)
    #    search_type = my.get_search_type()
    #    return SearchType.column_exists(search_type, column)
    ...
    def get_xml_value(my, name, root=None, strip_cdata=True):
    ...
    def add_related_sobject(my, sobject, src_path=None):
        '''add a related sobject.  This uses the new style relationships'''
        search_type = my.get_base_search_type()
    ...
    def commit(my, triggers=True):
        ...
        SObjectUndo.log_undo(my, id, is_insert)
        ...
        # autogenerate the code if none was specified
        if sobject and sobject.has_value("code") and not sobject.get_value("code"):
            # FIXME: make this configurable
            search_type = my.get_base_search_type()
            if search_type in ['sthpw/file', 'sthpw/snapshot', 'sthpw/transaction_log', 'sthpw/ticket', 'sthpw/task', 'sthpw/note','sthpw/work_hour']:
                padding = 8
            else:
                padding = 5
            code_expr = "%%s%%0.%dd" % padding
            new_code = code_expr % (table.upper(), id)
        ...
        if triggers and is_undo != True:
            try:
                ...
                schema = None
                sql.rollback_savepoint()
            if schema:
                if is_insert:
                    schema.insert_trigger(my)
                else:
                    schema.edit_trigger(my)

            # call a trigger
            # Certain search types are ignored because these are required in
            # an insert and will cause an infinite loop
            if my.get_base_search_type() not in ['sthpw/transaction_log', 'sthpw/ticket', 'sthpw/transaction_state', 'sthpw/access_log', 'sthpw/status_log']:
                ...
                if base_search_type in ["sthpw/note", "sthpw/task", "sthpw/work_hour", "sthpw/snapshot"]:
                    ...
                    # FIXME: this logic needs to be elsewhere
                    parts = parent_type.split("?")
                    parent_type = parts[0]
                else:
                    parent_type = my.get_base_search_type()
                my._call_triggers(trigger_update_data, mode, output, process, parent_type)
    ...
    def _call_triggers(my, trigger_update_data, mode, output, process, parent_type):
        ...
        for event in events:
            # first key
            key = {'event': event}
            Trigger.call_by_key(key, my, output)
    ...
    def set_defaults(my):
        ...
        if SearchType.column_exists(my.full_search_type, "timestamp") and not my.get_value("timestamp"):
            db_resource = my.get_db_resource()
            sql = DbContainer.get(db_resource)
    ...
    def delete(my, log=True):
        ...
        if log:
            SObjectUndo.log_undo_for_delete(my, db_resource)
    ...
    def children_alter_search(my, search, child_type):
        ...
        if schema:
            ...
            if relationship == "search_type":
                ...
                # TODO: relationship should be obtained from relationship_attrs
                ...
                # if connected through connection table, just return nothing for now
    ...
    def get_parent(my, parent_type=None, columns=[], show_retired=False):
        ...
        if parent_type == '*':
            ...
            if prefix:
                ...
                prefix = ""
            if my.has_value("%ssearch_type" % prefix) and my.has_value("%ssearch_id" % prefix):
...
class SObjectUndo:
    ...
    def log_undo(sobject, id, is_insert):

        # do not log the command log - DEPRECATED
        if sobject.get_search_type() == "sthpw/command_log":
            return
        ...
        Xml.set_attribute(sobject_node,"search_id", '%s'%id)
    ...
    def log_undo_for_delete(sobject, db_resource):
        '''this file handles the undo mode for delete of an sobject'''
        # do not log the command log - DEPRECATED

        search_type = sobject.get_search_type()
        if search_type == "sthpw/command_log":
            return
        ...
        sobject_node = transaction.create_log_node("sobject")
        Xml.set_attribute(sobject_node,"search_type",sobject.get_search_type())
        Xml.set_attribute(sobject_node,"search_id",sobject.get_id())

        Xml.set_attribute(sobject_node,"action","delete")

        column_types = []
        if db_resource.get_database_type() == 'SQLServer':
            column_types = SearchType.get_column_types(search_type)

        for key,value in data.items():
            # SQLServer
            if key == '_tmp_spt_rownum':
                continue
            if db_resource.get_database_type() == 'SQLServer':
                col_type = column_types.get(key)
                if col_type == 'sqlserver_timestamp':
                    continue
            node = xml.create_element("column")
            Xml.set_attribute(node,"name",key)
            if value == None:
    ...
    def undo(node):
        ...
        # but it actually wasn't.  The only know reason for this would be
        # that a commit failed due to referential integrity violation.
        # This is not reported until commit time.  This will at least allow
        # backout of that error
        ...
        if action == "insert":
            ...
            for column in columns:
                ...
                if database_type == 'SQLServer':
                    if SearchType.get_column_type(search_type, name) == 'datetime2' and value =='GETDATE()':
                        quoted = False
                elif database_type == 'Sqlite':
                    if SearchType.get_column_type(search_type, name) == 'timestamp' and value in ["CURRENT_TIMESTAMP", "datetime('now')"]:
                        quoted = False
                elif database_type == 'MySQL':
                    if SearchType.get_column_type(search_type, name) in ['timestamp', 'datetime'] and value in ["CURRENT_TIMESTAMP", "UTC_TIMESTAMP()"]:
                        quoted = False
            ...
            if sobject != None:
                ...
                print "Warning: Redoing delete: sobject does not exist!"
