...
class Sql(Base):
    ...
    def get_column_info(my, table, column=None):
        impl = my.get_database_impl()
...
# 
    ...
    # is used here because it appears that pgdb (whichi is DB-API 2.0 compliant)
...
class Select(object):
    ...
    def add_filter(my, column, value, column_type="", op='=', quoted=None, table=''):
        ...
        elif column_type == "integer" and value in ['true', 'false']:
            quoted = False
    ...
    def get_statement(my, mode="normal"):
        ...
        if mode == "count":
            ...
            if not my.columns:
                ...
                #statement += " " + ", ".join( ['"%s"'%x for x in my.columns] )
            if database_type =='SQLServer' and my.limit != None:
        ...
        if len(my.wheres) > 0:
            ...
            stack = []
            lstack = None
            lstack_storage = [] # storing for 2nd level manipulation

            for item in my.wheres:
                if item == 'begin':
                    if lstack:
                        lstack_storage.extend(lstack)
                    lstack = []

                elif item in ["and","or"]:
                    if lstack != None:
                        # if the stack has items, process them
                        if lstack:
                            op = " %s " % item.upper()
                            expr = "( %s )" % op.join(lstack)
                            stack.append(expr)
                        lstack = None
                        ...
                        b = stack.pop()
                        a = stack.pop()
                        expr = "( %s %s %s )" % ( a, item.upper(), b )
                        stack.append(expr)
                        if lstack_storage:
                            lstack_storage.extend(stack)
                            op = " %s " % item.upper()
                            expr = "( %s )" % op.join(lstack_storage)
                            stack = [expr]
                            lstack_storage = []
                    ...
                    if lstack != None:
                        ...
                        stack.append(item)
            # if there are any items laft on the lstack, add them,
            # this is usually the result of a dangling 'begin'
            if lstack:
                stack.extend(lstack)
            ...
            """
            # TEST NEW IMPLEMENTATION (ON now with new_expr)
            wheres_copy = my.wheres[:]
            new_expr = my.process_wheres(wheres_copy)
            """
            if new_expr != expr:
                print "---"
                print "WARNING: new where implementation and old do not match"
                print my.wheres
                print "new : ", new_expr
                print "old : ", expr
    ...
    def get_statement(my):
        ...
        for column, col_data in my.data.items():
            ...
            if primary_key:
                ...
                elif my.impl.get_database_type() in ['MySQL']:
                    expr = "%s auto_increment" % expr
