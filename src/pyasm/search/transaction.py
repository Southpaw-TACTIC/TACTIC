...
class FileUndo:
    ...
    def remove(src):
        ...
        FileUndo._add_to_transaction_log("remove", src, tmp_file)

    remove = staticmethod(remove)


    def create(orig, src=None, io_action=True):
        ...
        FileUndo._add_to_transaction_log("create", src, "")
    ...
    def symlink(src,dst,mode='symlink'):
        ...
        if os.name == "nt" or mode == 'copy':
            ...
            # TODO: make this all more portable.  These use absolute paths
            #rel_path = Common.relative_path(dst, src)
        ...
        FileUndo._add_to_transaction_log("symlink", src, dst, extra)
...
class TableUndo(Base):
    ...
    def redo(node):
        try:
            ...
            cmd = "psql -e -h %s -U %s %s < %s" % \
                (host, user, database, schema_path)
            os.system(cmd)
