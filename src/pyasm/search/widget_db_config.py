...
class WidgetDbConfig(SObject):
    ...
    # FIXME: search_type is not used here!!! Fix
    ...
    def set_value(my, name, value, quoted=True):
        if name == 'view':
            my.view = value
        return super(WidgetDbConfig, my).set_value(name, value, quoted=quoted)
    ...
    def get_by_search_type(search_type, view, project_code=None):
        if not project_code:
        ...
        # if there is not Widget config table
