...
class DatabaseImpl(object):
    ...
    # This relies on schema files.  The sql converter needs to
    # be called for this for all databases for each change.
    def import_schema(my, db_resource, type):
        ...
        for schema_type in types:
            ...
            print "Importing schema [%s] for database [%s]..." % (schema_type, database)
            for script in scripts:
                script = script.strip()
                if not script:
                    continue;
                sql.do_update(script)
...
class SQLServerImpl(DatabaseImpl):
    ...
    def process_value(my, name, value, column_type="varchar"):
        ...
        if column_type in ['timestamp','datetime']:
            ...
            # and not set_value
                ...
                pass
            elif not value.lower().startswith("convert") and not value.lower().startswith("getdate") and not value.lower().startswith("dateadd") :
                if value == 'NULL':
                    pass
                else:
    ...
    def get_schema_dir(my):
        ...
        # should be:
        #schema_path = "%s/src/schema/%s_schema.sql"
        schema_dir = "%s/src/pyasm/search/upgrade/sqlserver" % install_dir
    ...
    def import_schema(my, db_resource, type):
        ...
        for schema_type in types:
            ...
            #print "FINSIHED importin schema"
...
class PostgresImpl(DatabaseImpl):
    ...
    def get_schema_dir(my):
        ...
        # should be:
        #schema_path = "%s/src/schema/%s_schema.sql"
        schema_dir = "%s/src/pyasm/search/upgrade/schema" % install_dir
        if not os.path.exists(schema_dir):
            raise DatabaseImplException("Schema '%s' does not exist" % schema_dir)
...
class SqliteImpl(PostgresImpl):
    ...
    def get_schema_dir(my):
        ...
        # should be:
        #schema_path = "%s/src/schema/%s_schema.sql"
        schema_dir = "%s/src/pyasm/search/upgrade/sqlite" % install_dir
        if not os.path.exists(schema_dir):
            raise DatabaseImplException("Schema '%s' does not exist" % schema_dir)
    ...
    def import_schema(my, db_resource, type):
        ...
        for schema_type in types:
            ...
            # This is non-standard DBI - only Sqlite has this implemented
            print "Importing schema [%s] to database [%s]..." % (schema_type, database)
            print "   path [%s]" % (schema_path)
            conn = sql.conn
            try:
                conn.executescript(data)
            except Exception, e:
                # OSX SQLite complains about statements with only comments
                if str(e).find("provide a complete SQL statement") != -1:
                    pass
                else:
                    raise


    def import_default_data(my, database, type):
        '''import the data of certain type to the given database'''

        if not type:
            raise SetupException('No type specified for [%s] to import default data' % database)
        ...
        """
        raise Exception("import default data not implemented")
        schema = 'psql -q %s %s < "%s"' % (my._get_db_info(database), database, schema_path)
        print "Importing data ..."
        print schema
        os.system(schema)
        """
...
class MySQLImpl(PostgresImpl):
    ...
    # TODO: this is general enough for all databases
    # Need to test on PostgreSQL
    def get_schema_dir(my):
        # import the necessary schema
        install_dir = Environment.get_install_dir()
