...
class TableInfoCache(BaseCache):
    ...
    def init_cache(my):
        ...
        for table in my.tables:
            column_data = DatabaseImpl.get().get_column_info(my.database, table, use_cache=False)
        ...
        for table in my.tables:
            column_list = sql.get_columns(table, use_cache=False)
...
class CacheStartup(object):
    ...
    def start_cache_tasks(my, scheduler):
        ...
        # do a full cache refresh every 30 seconds
    ...
    def start_basic_tasks(my, scheduler):
        ...
        if not WebContainer.is_dev_mode():
            task = KillTacticTask()
            delay = Config.get_value("services", "process_time_alive")
            if delay:
                import random
                
                # put in a randomizer so that not all processes die at once
                delay = int(delay)
                offset = random.randint(0, delay) - delay/2
                delay += offset
                delay = delay * 60
                print "randomized delay:", delay
                scheduler.add_single_task(task, mode='sequential', delay=delay)
...
if __name__ == '__main__':
    ...
    print cache.get_value_by_key('data', 'login')
