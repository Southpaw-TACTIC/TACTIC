...
class HtmlElement(Widget):
    ...
    def add_gradient(my, name, palette_key, modifier=0, range=-20, reverse=False, default=None):
        ...
        if web.is_IE():
            ...
            if web.get_browser() == 'Mozilla':
                my.add_style("%s: -moz-linear-gradient(top, %s, %s)" % (name, color1, color2))
            else:
                my.add_style("%s: -webkit-gradient(linear, 0%% 0%%, 0%% 100%%, from(%s), to(%s))" % (name, color1, color2))
            return (color1, color2)

    def add_border(my, modifier=None, style=None, direction=None, color=None):
    ...
    def get_display(my):
        ...
        if my.behaviors:
            ...
            for c in range(len(my.behaviors)):
                if c:
                    bvr_str_list.append(',')
                bvr_spec_str = HtmlElement.get_json_string(my.behaviors[c])
    ...
    def set_box_shadow(my, value="0px 0px 3px 3px", color="#999"):
        browser = WebContainer.get_web().get_browser()
        if browser == 'Mozilla':
