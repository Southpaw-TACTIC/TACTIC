...
from pyasm.common import Environment, Xml, TacticException, Config, Common
    ...
    def filter_xml(my, xml):
        ...
        for node in img_nodes:
            ...
            if src.startswith("/plugins/"):
                plugin_dir = Environment.get_plugin_dir()
                path = "%s/%s" % (plugin_dir, src.replace("/plugins/", "") )
            ...
            if size[0] > 500:
                ...
                my.add_shadow(styles, "3px 3px 3px 3px", "#555")
                ...
                my.add_shadow(styles, "1px 1px 1px 1px", "#555")
...
class HelpWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        dialog.setStyle("left", size.x-670);
        dialog.setStyle("top", 40);
...
class HelpCreateWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        div.add(icon)
        div.add("Add a custom documentation page by clicking the Create button")
