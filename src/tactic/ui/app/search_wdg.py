...
class SearchWdg(BaseRefreshWdg):
    ...
    def get_default_filter_config(my):
        ...
        ''' % (my.search_type, my.search_type, custom_filter_view, my.search_type, my.search_type, my.search_type)
        """
        config_xml = Xml()
    ...
    def init(my):
        ...
        # instantiate a search limit
        #my.search_limit = SearchLimitWdg()

        # add a filter widget
        #tmp_path = __file__
        #tmp, base = my.search_type.split("/",1)
        #dir_name = os.path.dirname(tmp_path)
        #file_path="%s/config/%s-conf.xml" % (dir_name, base)

        # FIXME: this is still hard coded
        my.prefix = 'main_body'
    ...
    def get_retrieve_wdg(my):
        ...
        behavior = {
            'type':         'click',
            'cbfn_action':  'spt.dg_table.retrieve_search_cbk'
        }
    ...
    def get_save_wdg(my):
        ...
        behavior = {
            'type':         'click',
            'mouse_btn':    'LMB',
            'cbfn_action':  'spt.dg_table.save_search_cbk'
        }
...
class LocalSearchWdg(SearchWdg):
    ...
    def get_search_wdg(my):
        ...
        run_search_bvr = {
            'type':         'click_up',
            'cbfn_action':  'spt.dg_table.local_search_cbk',
            'panel_id':     my.prefix,
