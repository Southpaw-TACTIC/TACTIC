...
from pyasm.widget import ThumbWdg, SelectWdg, ButtonWdg, TextWdg, CheckboxWdg, IconWdg, PasswordWdg, HiddenWdg, HintWdg
...
class PageHeaderWdg(Widget):
    ...
    def get_display(my):
        ...
        if my.show_project:
            ...
            td.add_style("white-space: nowrap; padding-left: 14px")
        ...
        td.add(help_dialog)
        #help_dialog.set_as_activator(button, {'x': -270, 'y': 10} )
        ...
        tactic_header_div.add_gradient("background", "background2", 10)
        #tactic_header_div.add_style("padding-left: 5px")
        #tactic_header_div.add_style("padding-right: 5px")
        #tactic_header_div.set_round_corners(corners=['BR'])
        #tactic_header_div.add_style("margin: -3px")
...
class ProjectCreateWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        cb = CheckboxWdg('jump_project', label='Jump to New Project')
        ...
        'type': "click_up",
        'cbjs_action': '''
        var element = bvr.src_el.getParent(".spt_project_top");
        var values = spt.api.Utility.get_input_values(element, null, null, null, {cb_boolean: true});
        ...
        setTimeout( function() {
            ...
            spt.api.Utility.clear_inputs(element);

            // show feedback at the end
            if (values['jump_project'][0]== 'true') {
                setTimeout( function() {
                    document.location = "/projects/" +   project_code + "/link/_startup";
                    }, 1000);
            }
            else { 
                setTimeout("spt.panel.refresh('ProjectSelectWdg');", 2800);
                spt.app_busy.hide();
            }
...
class AppBusyWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        #rc_div = RoundedCornerDivWdg(hex_color_code="2F2F2F",corner_size="10")
        #rc_div.set_dimensions("350px","90px")

        rc_div = DivWdg()
        rc_div.add_style("width: 350px")
        rc_div.add_style("height: 70px")
        #rc_div.add_style("background: #2F2F2F")
        rc_div.add_gradient("background", "background", -60, range=-10)
        #rc_div.add_color("background", "background", -60)
        rc_div.add_style("padding: 10px")
        #shadow_color = rc_div.get_color("background")
        rc_div.set_round_corners(10)

        # get a subdued glow effect or shadow , depending on the background
        bg_color = rc_div.get_color('background', modifier=-10)
        rc_div.set_box_shadow(value = '0 0 8px', color=bg_color)
        #rc_div.set_box_shadow(color='#444')
        div.add( rc_div )
        ...
        #rc_div.get_content_wdg().add_style( "color: #FFF")
        rc_div.add_style( "color: #FFF")

        title_span = SpanWdg()
        title_span.add_class("spt_app_busy_title")
        title_span.add_styles("margin-left: 15px;")
        title_span.add_looks("fnt_title_4 fnt_bold")
        ...
        msg_span.add_looks("fnt_title_5")
