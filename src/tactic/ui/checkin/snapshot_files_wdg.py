...
class SnapshotDirListWdg(DirListWdg):
    ...
    def add_top_behaviors(my, top):
        ...
        if browser == 'Qt':
            ...
            if location == "scm":
                ...
                'bvr_match_class': 'spt_dir_list_item',
                'cbjs_action': '''
                var scm_base_url = "http://localhost:8080";

                var path = bvr.src_el.getAttribute("spt_path");
                version = bvr.src_el.getAttribute("spt_version");
                var url = scm_base_url+path+"?rev="+version;
                window.open(url);
                '''
                } )
            else:
        ...
        my.sandbox_dir = "C:/sthpw2/sandbox"


        menu_item = MenuItem(type='action', label='Copy to Sandbox')
        #menu.add(menu_item)
        ...
        #FIX ME: Added link in the menu but does the same thing as copy to clipboard
        menu_item = MenuItem(type='action', label='Copy to Windows Clipboard')
        menu.add(menu_item)
        menu_item.add_behavior( {
        'type': 'click_up',
        'cbjs_action': '''
        ...
        try {
            spt.app_busy.show("Copy to Windows Clipboard ...");
    ...
    def handle_item_div(my, item_div, dirname, basename):
        ...
        if snapshot:
            ...
            item_div.add_attr("spt_version", snapshot.get_value("version") )
...
class SnapshotMetadataWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        if not keys:
            ...
            # make sure all the keys are shown
            keys_set = set(keys)
        ...
        # TEST
        priority_keys = ['Format', 'Geometry', 'Depth', 'Colorspace']
    ...
    def get_shelf_wdg(my):
        ...
        file = my.snapshot
        lib_path = file.get_lib_path()

        if not file.get_value("metadata"):
            snapshot = file.get_parent()
            from pyasm.checkin import CheckinMetadataHandler
            metadata_handler = CheckinMetadataHandler(files=[lib_path],file_objects=[file], snapshot=snapshot)
            metadata_handler.execute()
        ...
        button = IconButtonWdg(title="Reread Metada", icon=IconWdg.EDIT)
        div.add(button)
        button.add_style("float: left")
        """
