...
class DiscussionWdg(BaseRefreshWdg):
    ...
    def add_layout_behaviors(cls, layout, hidden=False):
        ...
        layout.add_relay_behavior( {
            ...
            if (! add_note) {
                ...
                //spt.panel.load_popup("Add Note", class_name, kwargs, );
    ...
    def get_notes(my):
        ...
        if my.use_parent == 'true':
            if my.parent:
                ...
                if has_process:
                    key = "%s|%s|%s" % (my.parent.get_search_type(), my.parent.get_id(), parent_process)
                else:
    ...
    def get_display(my):
        ...
        if notes:
            ...
            # FIXME: this no longer works if it is not in a table!!

            # moved to handle_layout_behavior
            """
            expand_wdg.add_behavior( {
            'type': 'click_up',
            ...
            var title = "Notes: " + bvr.code;
            var element_name = bvr.search_key;
        ...
        # This only shows up if there are no notes
            ...
            add_wdg = ActionButtonWdg(title="+", title2="-", tip='Add a new note', size='small')
            #add_wdg = IconButtonWdg(title="Add New Note", icon=IconWdg.ADD)
            ...
            no_notes_msg.add_style("width: 90%")
            no_notes_msg.add_style("text-align: center")
            no_notes_div.add(no_notes_msg)
            no_notes_msg.add("<i>-- No notes found --</i>")
            no_notes_div.add_style("font-size: 10px")
            ...
            #add_note_wdg = DiscussionAddNoteWdg(parent=my.parent, context=context_choices, process=process_choice, use_parent=my.use_parent, append_process=my.append_processes)
    ...
    def get_context_wdg(my, process, context, show_context_notes=True):
        ...
        count_div.add_style("font-size: 11")
    ...
    def get_note_wdg(my, note, show_context_notes=True, note_hidden=False):
        ...
        if not show_context_notes and context not in my.default_contexts_open:
        ...
        # FIXME: something is blocking the checkbox from working
        """
        checkbox = CheckboxWdg("status")
        checkbox.set_attr("checked", "true")
        title.add(checkbox)
        checkbox.add_behavior( {
        'type': 'click_up',
        'cbjs_action': '''
        if (bvr.src_el.checked == false) {
        ...
        key = note.get_search_key(use_id=True)
        ...
        if note_hidden in ['true', True] and my.note_expandable in ['true', True]:
        ...
        context_div.add_style("font-size: 14px")
