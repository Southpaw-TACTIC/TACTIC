...
__all__ = ['CalendarWdg', 'CalendarMonthWdg', 'CalendarTimeWdg', 'CalendarInputWdg', 'TimeInputWdg', 'CalendarTimeInputWdg']
...
class CalendarWdg(BaseRefreshWdg):
    ...
    def init(my):
        ...
        if my.kwargs.get("show_border") not in ['false', False]:
            ...
            my.top_wdg.set_box_shadow("1px 1px 1px 1px")
...
class CalendarInputWdg(BaseInputWdg):
    ...
    ARGS_KEYS = {
        ...
        "time_input_default": {
        'description': 'The default for time applicable when show_time_is true or false. e.g. 5:00 PM',
        'type': 'TextWdg',
        'category': 'Display',
        'order': 3
        }
    ...
    def get_display(my):
        ...
        # FIXME: start week with Sunday ... this is specific to MMS.  May
        # want to globalize in config for all calendars?
        ...
        if read_only == 'true':
            ...
            text.add_event('onfocus', '''var el = $(this).getParent('.calendar_input_top').getElement('.spt_calendar_top'); spt.show(el);spt.body.add_focus_element(el)''')
...
# DEPRECATED
class TimeInputWdgOLD(BaseInputWdg):

    def get_display(my):

 
        div = FloatDivWdg()
        div.add_class("spt_time_top")


        my.time_cbk = my.get_option('time_cbk')
        show_activator = my.get_option('show_activator')
        if show_activator == "" or show_activator == True:
            show_activator = True
        else:
            show_activator = False
        ...
        # This widget is embedded when it is called from a calendar widget
        is_embedded = my.get_option("is_embedded")
        if is_embedded in ['true', True]:
            is_embedded = True
        else:
            is_embedded = False
            show_activator = False
        ...
        show_confirm = my.get_option('show_confirm')
        if show_confirm == "":
            # EditWdg doesn't have the confirm OK button
            from tactic.ui.panel import EditWdg
            if isinstance(my.get_parent_wdg(), EditWdg):
                show_confirm = False
            else:
                # for used in TableLayoutWdg or FastTableLayoutWdg
                show_confirm = True
            
        elif show_confirm in ['false', False]:
            show_confirm = False
        else:
            show_confirm = True

        # there is an assumption it is used beside a CalendarWdg
        if show_activator:
            icon = IconWdg("Date", IconWdg.DATE)
            div.add(icon)

            icon.add_behavior( {
                "type": "click",
                "cbjs_action": '''var el = bvr.src_el.getParent('.calendar_input_top').getElement('.spt_calendar_top');
                    spt.simple_display_toggle(el);
                '''
            } )
        ...
        interval = my.get_option("interval")
        if interval:
        ...
        if not value or is_expr:
            value = "12:00"

        parts = value.split(":")
        
        # keep them as strings to match the select string values
        hour = parts[0]
        min = parts[1]


        hour_name = my.get_input_name("%s|hour" % my.name)
        minute_name = my.get_input_name("%s|minute" % my.name)
        hr_select = SelectWdg(hour_name)
        hr_select.add_class('spt_time_hour')
        hr_select.set_value(hour)
        values =  [str(x).zfill(2) for x in xrange(0,24)]

        hr_select.set_option('values', values)
        hr_select.set_option('labels', values)
        div.add(hr_select)

        div.add(" : ")

        select = SelectWdg(minute_name)
        select.add_class('spt_time_minute')
        select.set_value(min)
        min_range = range(0, 60, interval)
        values_list = [str(x).zfill(2) for x in min_range]
        select.set_option('values', values_list )
        select.set_option('labels', values_list )
        div.add(select)
      
        show_confirm = False
        if show_confirm:
            ok = ProdIconButtonWdg('ok')
            ok.add_color('background','background2')
            ok.add_behavior({'type': 'click_up',
                'cbjs_action' : my.time_cbk})
            div.add(ok)
        else:
            # add to the hr/ minute select
            hr_select.add_behavior({'type': 'change',
                            'cbjs_action': my.time_cbk})
        ...
        if not is_embedded:
            ...
            # This is special code for table layout
            button = IconButtonWdg("OK", IconWdg.EDIT)
            div.add(button)
            button.add_behavior( {
                'type': 'click_up',
                'prefix': my.get_name(),
                'cbjs_action': '''
                var top_el = bvr.src_el.getParent(".spt_time_top");
                var values = spt.api.Utility.get_input_values(top_el);
                value_wdg = bvr.src_el;
                var hour = values[bvr.prefix+"|hour"];
                var minute = values[bvr.prefix+"|minute"];
                value_wdg.value = hour+":"+minute;
                spt.table.accept_edit(top_el, value_wdg.value, true);
                '''
            } )

        div.add_class("spt_no_alter")
...
# DEPRECATED
class CalendarTimeInputWdg(CalendarInputWdg):
    '''CalendarInput + TimeputWdg for datetime type column'''
   


    def get_display(my):

        my.set_option('show_time','true')
        return super(CalendarTimeInputWdg, my).get_display()
        """
        #TODO: if this widget is aware of its parent widget's layout version
        # we can target the proper javascript for the day_cbk and time_cbk
        
        my.top.add_class("calendar_input_top")

        # NOTE: start week with Sunday May want to globalize in config
        # for all calendars?
        first_day_of_week = my.get_option('first_day_of_week')
        if first_day_of_week == '':
            first_day_of_week = 6
        ...
        if show_activator == "" or show_activator in [True,'true']:
            show_activator = True
            show_text = True
        else:
            show_activator = False
        ...
        if show_activator:
            show_text = my.get_option('show_text')
            if show_text in [False, 'false']:
                show_text = False
            else:
                show_text = True
            ...
            today_icon = IconWdg("Today", IconWdg.ADD)
            today_icon.add_class('hand')
            ...
            today_icon = FloatDivWdg(today_icon)
            #today_icon.add_style("float: right")
            today_icon.add_class("spt_cal_input_today_btn")  # tag this button so we can find it to hide/show

        name = my.get_input_name()
        input = TextWdg( name=name )
        input.add_style("display: none")
        display_input = TextWdg( name="%s_display" % name )
        display_input.add_style("width: 70px")

        read_only = my.get_option('read_only')
        if read_only not in ['false', False]:
            read_only = 'true'
            #input.set_option("read_only", read_only)
            display_input.set_option("read_only", read_only)
        ...
        if my.value:
            input.set_value(my.value)
            display_input.set_value(my.value)
        current = my.get_current_sobject()
        if current and not current.is_insert():
            db_date = current.get_value(my.get_name())
            if db_date:
                date = Date(db=db_date)
                input.set_value(date.get_db_time(), set_form_value=False)
                display_input.set_value(date.get_db_time(), set_form_value=False)

        kbd_bvr = {
            'type': 'keyboard',
            'kbd_handler_name': 'CalendarInputKeyboard',
        }
        # this only matters with old table
        input.add_behavior( kbd_bvr )
        ...
        if not show_text:
            input.add_style('display: none')
            display_input.add_style('display: none')

        input.add_class("spt_calendar_input")
        display_input.add_class("spt_calendar_display")
        input_div = FloatDivWdg()
        input_div.add(input)
        input_div.add(display_input)

        input_div.add_style('margin-right: 5px')
        my.top.add(input_div)
        

        # calenar portion
        if show_activator:
            my.top.add(SpanWdg('&nbsp;',css='small'))
            #my.top.add(today_icon)
            ...
            var input_top = bvr.src_el.getParent('.calendar_input_top');
            var value = bvr.src_el.getAttribute('spt_date');
            var el = bvr.src_el.getParent('.calendar_input_top').getElement('.spt_calendar_display');
            el.value = value;
            var el = bvr.src_el.getParent('.calendar_input_top').getElement('.spt_calendar_input');
            el.value = value;
    
            var time_top = input_top.getElement('.spt_time_top');
            var time_values = spt.api.Utility.get_input_values(time_top);
            if ( value && value.test(/^\d\d\d\d-\d\d-\d\d( .*)?/) ) {
                 var parts = value.split(' ');
                 var new_value = parts[0] + ' ' + spt.zero_pad(time_values['%s|hour'], 2) + ':' + spt.zero_pad(time_values['%s|minute'], 2);
                 el.value = new_value;
            }
            ...
            calendar = CalendarWdg( name=name, first_day_of_week=first_day_of_week,day_cbk=day_cbk)
            ...
            if (version == "2") {
                spt.table.set_layout(layout);
                spt.table.accept_edit(input_top, new_value);
            }
            else {
                // A CellEditWdg containing a Calendar widget will be a detached PUW, so use 'spt.get_parent()'
                var td = spt.get_parent( el, '.spt_table_td' );
                if( td ) {
                    ...
                    //el.spt_text_edit_wdg.accept_edit = true;
                    //spt.dg_table.edit_cell_cbk(el, spt.kbd.special_keys_map.ENTER);
                    // Need to set it here since it is a Calendar time combo. and don't want to return to home cell yet
        ...
        time_cbk='''
            ...
            if (version == "2") {
                ...
                var time_top = input_top.getElement('.spt_time_top');
                var time_values = Utility.get_input_values(time_top);
                if (!value) {
                   var date_obj = new Date();
                   value = date_obj.getFullYear() + '-' + spt.zero_pad(date_obj.getMonth()+1, 2) +
                            '-' + spt.zero_pad(date_obj.getDate(), 2);
                }
                if ( value && value.test( /^\d\d\d\d-\d\d-\d\d( .*)?/ ) ) {
                     var parts = value.split(' ');
                     var new_value = parts[0] + ' ' + spt.zero_pad(time_values['%(name)s|hour'], 2) + ':' + spt.zero_pad(time_values['%(name)s|minute'], 2);
                     var text = Utility.get_input(input_top, '%(name)s','.spt_calendar_input');
                ...
                if( td ) {
                    ...
                    var text = Utility.get_input(td, '%(name)s','.spt_calendar_input');
                    var time_top = td.getElement('.spt_time_top');
                    var time_values = Utility.get_input_values(time_top);
                    // auto use today's date if it's not set
                    if (!value) {
                       var date_obj = new Date();
                       value = date_obj.getFullYear() + '-' + spt.zero_pad(date_obj.getMonth()+1, 2) +
                                '-' + spt.zero_pad(date_obj.getDate(), 2);
        ...
        time_input = TimeInputWdg(name=name)
        time_input.set_option('is_embedded', True)

        # this is for TimeInputWdg to be aware if it is being drawn in EditWdg 
        # or TableLayout
        time_input.set_parent_wdg(my.parent_wdg)
        time_input.set_option('time_cbk', time_cbk)
        time_input.set_option('show_activator', not show_activator)
        time_input.set_option('show_confirm', my.get_option('show_confirm'))
        interval = my.get_option('interval')
        if interval:
            time_input.set_option('interval', interval)
        my.top.add(time_input)
