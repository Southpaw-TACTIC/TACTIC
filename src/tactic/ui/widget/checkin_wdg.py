...
class CheckinWdg(BaseRefreshWdg):
    ...
    def get_title_wdg(my):
        ...
        title = "Check-in [%s]" % my.sobject.get_code() 
        if my.snapshot:
    ...
    def get_display(my): 
        ...
        info_panel.set_name("Check-in")
        tab.add(info_panel)
        history = CheckinHistoryWdg(search_key=my.search_key, context=my.context, sandbox_dir=default_sandbox_dir)
        ...
        if (transfer_mode == 'upload' || transfer_mode =='web') {
            ...
            for (var i = 0; i < file_paths.length; i++) {
                ...
                if (is_dir) {
                    ...
                    server.upload_file(file_paths[i], ticket);
                }
            }
            transfer_mode = 'uploaded';
        }
...
class FileSelectorWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        content = DivWdg()
        top.add(content)
        content.add_style("margin: 5px 5px 0px 5px")
...
class CheckinSandboxListWdg(BaseRefreshWdg):
    ...
    def get_button_action_wdg(my):
        ...
        menu_item = MenuItem(type='action', label='Check-out File')
        menu.add(menu_item)
        menu_item.add_behavior( {
        'type': 'click_up',
        'sandbox_dir': my.base_dir,
        ...
        menu_item = MenuItem(type='action', label='Check-out all w/ Repository Name')
        menu.add(menu_item)
        menu_item.add_behavior( {
        'type': 'click_up',
        'sandbox_dir': my.base_dir,
