...
from pyasm.common import Xml, XmlException, Common, TacticException, Environment, Container
...
class CustomLayoutWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        if include_mako not in ['false', False]:
            ...
            """
            NOT NEEDED, with proper encoding specified
            special_chars = [167, 194]
            for sc in special_chars:
                html = html.replace(chr(sc), "&%s;" % sc)


            for sc in special_chars:
                html = html.replace("&amp;%s;" % sc, chr(sc))
            """
    ...
    def get_element_wdg(my, xml, def_config):
        ...
        try:
            ...
            # TODO: some protection code for infinite loops
