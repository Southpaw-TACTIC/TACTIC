...
#
#
__all__ = ["TableLayoutWdg", "AddPredefinedColumnWdg"]
...
class TableLayoutWdg(BaseConfigWdg):
    INSERT_ROW = 0
    EDIT_ROW = 1
    GROUP_WEEKLY = "weekly"
    ...
    def __init__(my, **kwargs):
        ...
        super(TableLayoutWdg,my).__init__(search_type=my.search_type, config_base=my.view, config=config)
    ...
    def get_display(my):
        ...
        """
        # add the click up behavior to the table for each cell using a
        # smart click_up
        behavior = {
            'type': 'smart_click_up',
            'mouse_btn': 'LMB',
            'bvr_match_class': 'SPT_CELL_EDIT',
            #'cbfn_action': 'spt.dg_table.select_cell_for_edit_cbk',
            'cbjs_action': 'spt.dg_table.select_cell_for_edit_cbk(evt, bvr, mouse_411)',
        ...
        for row, sobject in enumerate(my.sobjects):
            ...
            # set some attributes
            tbody.add_attr("spt_class_name", "tactic.ui.panel.TableLayoutWdg")
        ...
        if my.chunk_iterations:
            if my.chunk_num == 0:
                behavior = {
                    ...
                    setTimeout(function() {
                    var server = TacticServerStub.get();
                    var widget_class = 'tactic.ui.panel.TableLayoutWdg';
                    var args = {
                        search_type: '%s',
                        view: '%s',
                        search_view: '%s',
...
class SwitchLayoutMenu(object):
    ...
    def __init__(my, **kwargs):
        ...
        views = ['tile', 'list', 'content', 'navigate', 'schedule', 'checkin', 'tool', 'browser', 'overview']
        labels = ['Tile', 'List', 'Content', 'Navigator', 'Task Schedule', 'Check-in', 'Tools', 'File Browser', 'Overview']
...
class RowSelectWdg(BaseTableElementWdg):
    ...
    def handle_th(my, th, cell_idx=None):
        ...
        th.add_behavior( {'type': 'click_up', 'mouse_btn':'LMB', 'modkeys':'',
                          ...
                          'cbfn_action': 'spt.dg_table.select_deselect_all_rows'} )
...
class AddPredefinedColumnWdg(BaseRefreshWdg):
    ...
    def get_columns_wdg(my, title, element_names):
        ...
        #num_items_wdg = DivWdg()
        #title_wdg.add(num_items_wdg)
        #num_items_wdg.add(" (%s items)" % len(element_names) )
        #num_items_wdg.add_style("font-weight: italic")
        #num_items_wdg.add_style("font-size: 10px")
    ...
    def get_display(my):
        ...
        context_menu.add_style("padding: 10px")
        context_menu.add_border()
        context_menu.add_color("color", "color")
        context_menu.add_color("background", "background")
        ...
        context_menu.add( my.get_columns_wdg(title, defined_element_names) )



        # Add predefined columns

        # get predefined elements defined by plugins
        # TODO: should maybe be it's own category???
