...
class PopWindowWdg(BaseRefreshWdg):
    ...
    def init(self):
        ...
        if width:
            if type(width) == str:
                self.content_width = int( width.replace('px','') )
            else:
                self.content_width = width
        if height:
            if type(height) == str:
                self.content_height = int( height.replace('px','') )
            else:
    ...
    def get_display(self):
        ...
        popwin_div = DivWdg()
        if self.top_id:
            popwin_div.set_id( self.top_id )
        popwin_div.add_class("SPT_PWIN_TOP_DIV SPT_RSW_OUTER_TOP SPT_PUW spt_popup spt_pwin_DX spt_pwin_DY")
        popwin_div.set_z_start( self.z_start )
        popwin_div.add_styles( self.set_style( "display: none; position: absolute; top: 200px; left: 300px; " \
                                             "width: %spx; height: %spx;" % (full_w, full_h)) )

        left_div = DivWdg()
        left_div.add_class("spt_pwin_A spt_pwin_DY")
        left_div.add_styles( self.set_style("float: left; width: %spx; height: %spx;" % (shd_w, full_h)) )

        center_div = DivWdg()
        center_div.add_class("spt_pwin_B spt_pwin_DX spt_pwin_DY")
        center_div.add_styles( self.set_style("float: left; width: %spx; height: %spx;" % (cw, full_h)) )

        right_div = DivWdg()
        right_div.add_class("spt_pwin_C spt_pwin_DY")
        right_div.add_styles( self.set_style("float: left; width: %spx; height: %spx;" % (shd_w, full_h)) )
        ...
        lt_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, shd_h)) )
        lt_div.add_class("css_shadow_top_left spt_pwin_A_1")

        adj_h = ch + title_h
        if not is_IE:
            adj_h = adj_h + (2 * border_sz)
        lm_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, adj_h)) )
        lm_div.add_class("css_shadow_left spt_pwin_A_2 spt_pwin_DY")

        lb_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, shd_h)) )
        ...
        for wdg in self.added_widgets:
            center_resize_scroll_wdg.add( wdg )

        center_bottom_div = DivWdg()

        center_top_div.add_styles( self.set_style("width: %spx; height: %spx;" % (cw, shd_h)) )
        ...
        center_title_div.add_styles( self.set_style("cursor: move; border: %spx solid black; " \
        ...
        center_bottom_div.add_styles( self.set_style("width: %spx; height: %spx;" % (cw, shd_h)) )
        ...
        rt_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, shd_h)) )
        rt_div.add_class("css_shadow_top_right spt_pwin_C_1")

        adj_h = ch + title_h
        if not is_IE:
            adj_h = adj_h + (2 * border_sz)
        rm_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, adj_h)) )
        rm_div.add_class("css_shadow_right spt_pwin_C_2 spt_pwin_DY")

        rb_div.add_styles( self.set_style("width: %spx; height: %spx;" % (shd_w, shd_h)) )
...
class ResizeScrollWdg(BaseRefreshWdg):
    ...
    def init(self):
        ...
        self.affects_outside_flag = False
        if self.kwargs.get('affects_outside_flag'):
            self.affects_outside_flag = True

        self.scroll_bar_size_str = "medium"
        if self.kwargs.get('scroll_bar_size_str'):
            self.scroll_bar_size_str = self.kwargs.get('scroll_bar_size_str')

        self.scroll_expansion = "inside"
        if self.kwargs.get('scroll_expansion'):
            self.scroll_expansion = self.kwargs.get('scroll_expansion')

        self.exclude_top_border = self.kwargs.get('exclude_top_border')
        self.no_resize = self.kwargs.get('no_resize')

        self.min_content_w = self.kwargs.get('min_content_w')
        if not self.min_content_w:
            self.min_content_w = 150
        self.min_content_h = self.kwargs.get('min_content_h')
        if not self.min_content_h:
            self.min_content_h = 100

        self.max_content_w = self.kwargs.get('max_content_w')
        if not self.max_content_w:
            self.max_content_w = 0
        self.max_content_h = self.kwargs.get('max_content_h')
        if not self.max_content_h:
            self.max_content_h = 0

        self.set_max_to_content_size = self.kwargs.get('set_max_to_content_size')

        self.added_widgets = []


    def _get_scroll_bar_pixel_size(self):

        size_map = { 'thin': 8, 'medium': 12, 'thick': 16 }
        pixel_sz = size_map.get('medium')
        if size_map.get( self.scroll_bar_size_str ):
            pixel_sz = size_map.get( self.scroll_bar_size_str )
        return pixel_sz
    ...
    def get_display(self):
        ...
        if self.no_resize:
            resize_sz = 1
        scroll_sz = self._get_scroll_bar_pixel_size()
        ...
        if not is_IE:
            ...
            if not self.exclude_top_border:
                border_adj_in_ch = in_ch - border_sz
                border_adj_ch = ch - border_sz

        top_div = DivWdg()
        top_div.add_styles( self.set_style("width: %spx; height: %spx;" % (cw, ch)) )
        ...
        if not self.affects_outside_flag:
            top_div.add_class("SPT_RSW_OUTER_TOP")

        top_div.set_attr("spt_min_content_w","%d" % self.min_content_w)
        top_div.set_attr("spt_min_content_h","%d" % self.min_content_h)

        top_div.set_attr("spt_max_content_w","%d" % self.max_content_w)
        top_div.set_attr("spt_max_content_h","%d" % self.max_content_h)

        top_div.set_attr("spt_set_max_to_content_size","false")
        if self.set_max_to_content_size:
            top_div.set_attr("spt_set_max_to_content_size","true")

        top_div.set_attr("spt_scroll_size", scroll_sz)
        top_div.set_attr("spt_scroll_expansion", self.scroll_expansion)
        ...
        B2_i.add_styles( self.set_style("width: %spx; height: %spx;" % (cw, border_adj_in_ch)) )
        B2_ii.add_styles( self.set_style("width: %spx; height: %spx;" % (cw, resize_sz)) )
        ...
        B2_i_a.add_styles( self.set_style( ' '.join( content_styles ) ) )
        ...
        B2_i_b.add_styles( self.set_style("background-color: #444; float: left; width: %spx; " \
        ...
        if not is_IE:
            y_scroll_w -= 1
        y_scroll.add_styles( self.set_style("background-color: #949494; float: left; width: %spx; " \
        ...
        x_resize.add_styles( self.set_style("float: left; width: %spx; " \
                                          "height: %spx;" % (resize_sz, border_adj_in_ch)) )
        ...
        B2_ii_a.add_styles( self.set_style("background-color: black; float: left; width: %spx; " \
        ...
        if not is_IE:
            x_scroll_h -= 1
            x_scroll_w -= 1
        x_scroll.add_styles( self.set_style("background-color: #949494; float: left; width: %spx; " \
        ...
        y_resize.add_styles( self.set_style("float: left; width: %spx; " \
                                          "height: %spx;" % (in_cw, resize_sz)) )
        ...
        if self.no_resize:
            B2_ii_b.add_styles( self.set_style("background-color: black; float: left; " \
        ...
        #{
            ...
            B2_ii_b.add_styles( self.set_style("background-color: #434343; float: left; cursor: se-resize; " \
