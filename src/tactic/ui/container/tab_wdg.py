...
spt.tab.header_drag_motion = function( evt, bvr, mouse_411) {
    ...
    var dx = mouse_pos.x - spt.tab.last_header_pos.x;
    var dy = mouse_pos.y - spt.tab.last_header_pos.y;
    spt.tab.last_header_pos = mouse_pos;

    spt.pipeline.move_by(header, dx);
}
        '''
    ...
    def get_display(my):
        ...
        for element_name in element_names:
            ...
            header = my.get_tab_header(element_name, title, display_class, display_options, is_selected=is_selected, is_loaded=is_loaded)
        ...
        for i, widget in enumerate(my.widgets):
            ...
            header = my.get_tab_header(name, title, class_name, kwargs, is_selected=is_selected, is_loaded=True)
            header_div.add(header)
        ...
        content_top.add_style("min-height: 400px")
        ...
        content_top.add_color("background", "background", -20)
        ...
        title = ""
        is_selected = False
        header = my.get_tab_header(name, title, None, None, is_selected=is_selected)
    ...
    def get_tab_header(my, element_name, title, class_name=None, kwargs=None, is_selected=False, is_loaded=False):
        ...
        if is_selected:
            ...
            header.add_gradient("background", "background", 0)
        ...
        palette = header.get_palette()
        hover_color = palette.color("background", 30)
        header.add_behavior( {
        'type': 'hover',
        ...
        if kwargs:
            # FIXME: this kwargs processing is a big HACK ... need to extract
            # what add_behavior does.
