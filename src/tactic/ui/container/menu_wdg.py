...
class FingerMenuWdg(BaseRefreshWdg):
    ...
    def set_activator_over(my, item, activator_match_class, activator_parent_class='', js_action='', top_class='', offset={'x':0, 'y':0}):
        ...
        main_action = '''
            ...
            if (panel != null) {
                var panel_pos = panel.getPosition();
                pos = {x: pos.x - panel_pos.x + bvr.offset.x, y: pos.y - panel_pos.y + bvr.offset.y};
            }

            var size = activator_parent.getSize();
            var x_offset = size ?  size.x : 400;
            var client_width = document.body.clientWidth;
            var scroll_x = document.getScroll().x;
            /*
            console.log("offset_X " + x_offset)
            console.log("pos X " + pos.x)
            console.log("scroll X " + scroll_x)
            ...
            */
            ...
            else if ((x_offset+ pos.x -scroll_x + menu_width) > client_width ) {
                is_left = true;
            }
