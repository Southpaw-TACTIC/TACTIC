...
#
#
__all__ = ["FreeFormCanvasWdg", "FreeFormLayoutWdg", "FreeFormLayoutToolWdg", "FreeFormAttrWdg"]
...
class FreeFormLayoutWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        for sobject in sobjects:
            #freeform_layout = FreeFormCanvasWdg(search_type=search_type, view=view, config_xml=config_xml)
            freeform_layout = my.get_canvas_display(search_type, view, config_xml, sobject) 
            freeform_layout.set_round_corners()
            top.add(freeform_layout)

        return top


    def get_canvas_display(my, search_type, view, config_xml, sobject):
        ...
        config = WidgetConfig.get(view='freeform', xml=config_xml)
...
class FreeFormLayoutToolWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        values = ['Label', 'Text', 'TextArea', 'Button', 'Preview', 'HTML', 'Data', 'Table']
        names = ['label', 'text', 'textarea', 'button', 'preview', 'html', 'data', 'table']
        ...
        freeform_layout = FreeFormCanvasWdg(search_type=search_type, view=view, config_xml=template_config_xml)
        template_div.add(freeform_layout)


        # handle the canvas
        canvas = table.add_cell()
...
<element name='table' xpos='100px' ypos='200px'>
  <display class='tactic.ui.panel.TableLayoutWdg'>
    <search_type>sthpw/virtual</search_type>
    <view>table</view>
  </display>
</element>
...
class FreeFormCanvasWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        // FIXME: search type and view aren't going through smart click
        bvr.search_type = 'TF3_224/shot';
        bvr.view = 'freeform';

        //var class_name = 'tactic.ui.manager.ElementDefinitionWdg';
        var class_name = 'tactic.ui.manager.SimpleElementDefinitionWdg';
        ...
        for element_name in element_names:
            ...
            buttons.setStyle("border", "solid 1px green");
            ''',
            'cbjs_action_out': '''
            var buttons = bvr.src_el.getElement(".spt_freeform_button_top");
            spt.hide(buttons);
            ...
            buttons_div.add_style("left: -20px")
            buttons_div.add_style("height: 100%")
            buttons_div.add_style("width: 100%px")
...
spt.freeform.resize_drag_setup = function(evt, bvr, mouse_411) {
    ...
    console.log(spt.freeform.resize_el);
...
spt.freeform.export = function() {
    ...
    console.log("num: "+elements.length);
...
class FreeFormAttrWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        delete = ActionButtonWdg(title='Delete')
        top.add(delete)
