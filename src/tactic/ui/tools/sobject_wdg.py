...
class SObjectDetailWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        #sobject_info_wdg.add_style("margin: 10px")


        td.add(sobject_info_wdg)
        #sobject_info_wdg.add_color("background", "background", -5)
        #sobject_info_wdg.add_style("width: 100%")
        #sobject_info_wdg.add_style("padding: 10px")
        ...
        #notes_div.add_style("min-height: 200")
        ...
        # right
        #if my.pipeline_code != 'default':
        #    td = table.add_cell()
        #    td.add_style("text-align: left")
        #    td.add_style("vertical-align: top")
        #    td.add_class("spt_pipeline_wrapper")
        #    td.add( my.get_pipeline_wdg(my.pipeline_code) )
        #    col_span = 3
        #else:
        #    col_span = 5
        col_span = 5
        ...
        #td.add_style("padding: 10px 20px 10px 20px")
        ...
        # disabling for now.
        #shelf = my.get_shelf_wdg()
        #shelf.add_style("margin-bottom: 5px")
        #td.add( shelf )
        ...
        tab = TabWdg(config=config, state=state, show_add=False )
        td.add(tab)
    ...
    def get_sobject_detail_wdg(my):
        ...
        config = WidgetConfigView.get_by_search_type(search_type=my.search_type, view=view)
        ...
        edit = EditLayoutWdg(search_type=my.search_type, mode='view', view="detail", search_key=my.search_key, width=400, title=' ', ignore=ignore, element_names=element_names)
        edit_div.add(edit)
...
class TaskDetailWdg(SObjectDetailWdg):
    ...
    def get_shelf_wdg(my):
        ...
        from tactic.ui.widget import DiscussionAddNoteWdg, SingleButtonWdg, ButtonNewWdg, ButtonRowWdg
        from tactic.ui.container import DialogWdg


        button_row = ButtonRowWdg()
        shelf.add(button_row)

        add_note_button = ButtonNewWdg(title='Add Note', icon=IconWdg.EDIT)
        button_row.add(add_note_button)

        add_note_dialog = DialogWdg()
        add_note_dialog.add_title("Add a new note")
        shelf.add(add_note_dialog)
        add_note = DiscussionAddNoteWdg(parent=my.parent, display=True)
        add_note_dialog.add(add_note)
        add_note_dialog.set_as_activator(add_note_button)


        change_status_button = ButtonNewWdg(title='Change Task Status', icon=IconWdg.NOTE)
        button_row.add(change_status_button)

        change_status_dialog = DialogWdg()
        change_status_dialog.add_title("Change Task Status")
        shelf.add(change_status_dialog)

        change_status_div = DivWdg()
        change_status_div.add_style("padding: 10px")
        change_status_div.add_color("color", "color")
        change_status_div.add_color("background", "background")
        change_status_div.add_border()
        change_status_div.add("Change status of task")

        change_status_dialog.add(change_status_div)
        change_status_dialog.set_as_activator(change_status_button)


        #whatever_button = ButtonNewWdg(title='Change Task Status', icon=IconWdg.FILM)
        #button_row.add(whatever_button)
...
class SObjectSingleProcessDetailWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        task_wdg = TableLayoutWdg(search_type="sthpw/task",view='single_process', show_row_select="false", show_insert="false", show_gear="false", show_search_limit="false", show_search=False, show_refresh="false")
        ...
        snapshot_wdg = TableLayoutWdg(search_type="sthpw/snapshot",view='table', mode='simple', show_row_select=False, width='100%')
