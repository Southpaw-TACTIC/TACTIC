...
class SchemaToolWdg(PipelineToolWdg, PipelineEditorWdg):
    ...
    def get_shelf_wdg(my):
        ...
        server.start( {description: 'Saving Schema for project ['+project_code+']'} );
    ...
    def get_tab_wdg(my):
        ...
        tab = TabWdg(config_xml=config_xml, show_add=False, tab_offset="10px")
...
class SchemaConnectorWdg(BaseRefreshWdg):
    ...
    def get_many_to_many_options(my, left_search_type_sobj, right_search_type_sobj, left_table, right_table, left_selected, right_selected, left_columns, right_columns):
        ...
        msg_div = DivWdg()
        div.add(msg_div)
        msg_div.add("This will create a new intermediate sType called<br/>which will relate the two sTypes.<br/>")
        msg_div.add_style("margin: 15px")
        msg_div.add_style("padding: 15px")
        msg_div.add_style("inherit: false")
        msg_div.add_border()
        msg_div.add_color("color", "color2")
        msg_div.add_color("background", "background2")
