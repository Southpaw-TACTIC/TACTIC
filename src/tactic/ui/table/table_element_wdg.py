...
class CheckoutButtonElementWdg(ButtonElementWdg):
    ...
    def get_display(my):
        ...
        // Hacky way to get sandbox_dir in checkin wdg
        if (kwargs.sandbox_dir == 'None') {
            kwargs.sandbox_dir = 'C:/sthpw/sandbox/project/video/asset002/design';
        }
        ...
        // HACK: reusing checkin transfer mode
            ...
            if (bvr.snapshot_code) {
                if (!bvr.checkout_script_path){
                    ...
                    setTimeout( function() {
                        try {
                            var server = TacticServerStub.get();
                            server.checkout_snapshot(bvr.snapshot_code, bvr.sandbox_dir, {mode: transfer_mode});
                        }
                        catch(e) {
