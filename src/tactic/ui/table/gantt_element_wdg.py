...
__all__ = [ 'GanttElementWdg', 'GanttCbk' ]
...
from pyasm.widget import HiddenWdg, TextWdg
...
class GanttElementWdg(BaseTableElementWdg):
    ...
    def preprocess(my):
        ...
        for index, properties in enumerate(my.property_list):
            ...
            if sobject_expr:
                ...
                gantt_data = expr_parser.get_flat_cache()
        ...
        # FIXME: the visible width should be settable
    ...
    def get_title(my):
        ...
        inner_div.add( my.get_month_wdg(my.start_date, my.end_date) )
    ...
    def get_dates_display(my, dates, date_expr, record_divider_pos=False,name=None, is_scalable=False):
        ...
        for i, date in enumerate(dates[:-1]):
            ...
            div.add_attr("spt_gantt_pixels_start", pixels_start)
            div.add_attr("spt_gantt_pixels_end", pixels_end)

            # handle the contents of the display
            title_div = DivWdg(display)
            if pixels_start + my.offset_width < 0:
                padding = -my.offset_width - pixels_start + 15

                # always center the months
                if 'MONTH' in date_expr:
                    title_div.add_style("text-align: center")
                
                elif padding <= pixels / 2:
                    title_div.add_style("padding-left: %s" % padding)
                else:
                    title_div.add_style("text-align: center")
            else:
                title_div.add_style("text-align: center")
    ...
    def get_display(my):
        ...
        for index, properties in enumerate(my.property_list):
            ...
            for range_data in sobject_data:
                ...
                bar = my.draw_bar(index, key, cur_color, editable, default=default, height=height, range_data=range_data)
    ...
    def draw_bar(my, count, key, color='grey', editable=True, default='now', height=5, range_data=None):
        ...
        duration = my.get_duration_wdg( duration_width, color, height, corners=corners)
        widget.add(duration)
        diff = end_sobj_date - start_sobj_date
        days = diff.days + 1
        duration.add("%s days" % days)

        duration.set_attr("title", "%s - %s (%s days)" % (start_display_date, end_display_date, days) )
    ...
    def get_duration_wdg(my, duration_width, color, height, opacity=0.8, top_margin=0, position='relative', left=None, corners=['TL','TR','BL','BR']):
        ...
        top_margin = 8 - (height/2) + top_margin
        if top_margin > 0:
        ...
        #duration.set_round_corners(5, corners=['TL','BL'])
        duration.set_round_corners(3, corners)
    ...
    def get_special_day_wdg(my):
        ...
        # add todays divider
        draw_div = my.get_day_bar_wdg(datetime.datetime.today(), "blue")
    ...
    def get_divider_wdg(my):
        ...
        for i, divider_width in enumerate(my.divider_pos):
            ...
            #divider_div = DivWdg()
            #divider_div.add_style("width: %s%%" % divider_width)
            #divider_div.add_style("float: left")
            #divider_div.add_style('-moz-user-select: none') 
            #divider_div.add_style('user-select: none') 
            #divider_div.add_style("text-align: left")
        ...
        dates = list(rrule.rrule(rrule.WEEKLY, byweekday=4, dtstart=my.start_date, until=my.end_date))
        if len(dates) > 75:
    ...
    def get_day_bar_wdg(my, date, color, opacity=0.5, days=1):
        ...
        draw_div.add_style("height: 100%")
        draw_div.add_style("height: 1000px")
