...
class PipelineEditWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        if not pipelines:
            ...
            div.add(icon)
            div.add("<b>This Searchable Type has no pipelines defined</b>")
            div.add("<br/>"*2)
...
class PipelineEditCbk(Command):
    ...
    def execute(my):
        ...
        for pipeline_code, pipeline_data in data.items():
            ...
            # get hte task pipeline
            task_pipeline = Pipeline.get_by_code("task")
            task_processes = task_pipeline.get_process_names()
            task_processes_str = ",".join(task_processes)
            ...
            for process, status in zip(processes, statuses):
                ...
                # check that the status list does not match the default.  If
                # it matches, then don't do anything
                if status == task_processes_str:
                    continue
