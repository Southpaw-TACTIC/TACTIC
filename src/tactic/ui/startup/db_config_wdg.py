...
class DbConfigContentWdg(BaseRefreshWdg):
    ...
    def get_display(my):
        ...
        if os.name == "nt":
            options = ['asset_base_dir', 'win32_server_handoff_dir', 'win32_client_handoff_dir','linux_client_handoff_dir']
        else:
            options = ['asset_base_dir', 'linux_server_handoff_dir', 'win32_client_handoff_dir','linux_client_handoff_dir']
...
class DbConfigSaveCbk(Command):
    ...
    def configure_asset_dir(my):
        ...
        linux_server_handoff = web.get_form_value("checkin/linux_server_handoff_dir")
        if linux_server_handoff:
            Config.set_value("checkin", "linux_server_handoff_dir", linux_server_handoff)
        else:
            Config.remove("checkin", "linux_server_handoff_dir")


        win32_client_handoff = web.get_form_value("checkin/win32_client_handoff_dir")
        if win32_client_handoff != None:
            Config.set_value("checkin", "win32_client_handoff_dir", win32_client_handoff)
        else:
            Config.remove("checkin", "win32_client_handoff_dir")
