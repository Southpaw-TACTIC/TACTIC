    ...
    def execute(my):
        ...
        if not script_sobj:
            try:
                ...
                script_sobj = search.get_sobject()
                if not script_sobj:
                    print("WARNING: Script with path [%s] does not exist in this project [%s] or Admin Site" % (project.get_code(), my.script_path))
                    return {}
            except:
                print("WARNING: Script with path [%s] does not exist in this project or Admin Site" % (project.get_code, my.script_path))
                return
