...
''' % code
        ...
        try:
            ...
            template.render(server=server,spt_mako_results=spt_mako_results, **my.kwargs)
        except Exception, e:
    ...
    def execute(my):
        ...
        if script_path:
            desc = 'Running PythonCmd with script path [%s]'%script_path
        else:
            desc = "Running PythonCmd with explicit code"
