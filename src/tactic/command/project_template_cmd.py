...
class ProjectTemplateCreatorCmd(Command):
    ...
    def export_template(my):
        ...
        #xml.set_attribute(manifest_node, "code", my.plugin_code)
        ...
        config_search_types = Search.eval("@GET(sthpw/search_object['namespace','%s'].search_type)" % namespace)
        ...
        for search_type in config_search_types:
        ...
        plugin.set_value("code", "%s_project" % my.project_code)

        from plugin import PluginCreator

        base_dir = Environment.get_template_dir()

        creator = PluginCreator( base_dir=base_dir, plugin=plugin, manifest=xml.to_string(), force=True)
        creator.execute()
...
class ProjectTemplateInstallerCmd(Command):
    ...
    def import_template(my):
        ...
        # if the template dir does not exist, then see if the template project exists in db
        if my.force_database or not template_dir:
            # check to see if this template project is in the database
           
            template = Project.get_by_code(my.template_project_code)
            
            if template:
                if not template_dir:
                    raise TacticException("Please press the Update button for this project [%s] in the Template Projects tab" %my.template_code)
                pass
                # this is now manually controlled by the user in the Template Projects tab.
                """
                cmd = ProjectTemplateCreatorCmd(project_code=my.template_project_code)
                cmd.execute()
                """
            else:
                if not os.path.exists(template_dir):
                    raise TacticException("Project template [%s] is not installed. Ensure the zip file is placed in the templates directory. And you need to run the -f <zip_path> option if using create_template.py" % my.template_code)


        # read the manifest file
        manifest_path = "%s/manifest.xml" % (template_dir)
        my.xml = Xml()
        my.xml.read_file(manifest_path)

        my.filter_manifest(my.xml)


        # check to see if the database exists
        # It is assumed that the project is created in the default database
        ...
        # but that break the ties to the project_type entry created though, which is ok
    ...
    def handle_path(my, src_path):
        ...
        """
        # TODO: The ZipUtil module errors when extracting the zip file.
        # Error:  unpack requires a string argument of length 46
        if os.name != 'nt':
            paths = ZipUtil.extract(target_path)
        else:
            src_path_without_zip_nor_version = src_path[0:-10]
            if not os.path.exists(src_path_without_zip_nor_version):

                print
                print 'To install a project template plugin on a Windows machine,'
                print 'please first unzip the TACTIC plugin file to its current directory:'
                print
                print '       ' + src_path_without_zip_nor_version
                print
                print 'and try again.'
                exit()
            shutil.move(src_path_without_zip_nor_version, upload_dir)
        """
        # veryify that the paths extracted are the expected ones
        rootname, ext = os.path.splitext(basename)
        #for path in paths:
        #    if not path.startswith("%s/" % rootname):
        #        raise TacticException("File in zip is not contained in [%s]" % rootname)

        #plugin_path = "%s/%s" % (plugin_base_dir, basename)
