...
spt.Applet = function()
{
    ...
    this.install_python = function() {
        ...
        var python = base + "/api/" + api_version + "/Scripts/python.exe";
    ...
    this.exec_local_cmd = function(class_name, kwargs)
    {
        ...
        if (! this.exists(python)) {
            if (confirm("TACTIC client side libraries not installed ... do you wish to install?")) {
                this.install_python();
            }
            else {
...
spt.Applet.get = function() {
    ...
    if (typeof(pyApplet) != 'undefined' && qt_browser) {
    ...
    try {
        applet.onload = function() {document.general_applet.init(); document.general_applet.exists();}
