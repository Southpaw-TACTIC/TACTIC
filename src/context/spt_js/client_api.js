...
TacticServerStub = function() {
     ...
     */
    this.upload_file = function(file_path, ticket) {
        if (typeof(ticket) == 'undefined') {
            ticket = this.login_ticket;
        }
        var applet = spt.Applet.get();
        applet.upload_file(file_path, ticket);
    }
    ...
    this.checkout_snapshot = function(search_key, sandbox_dir, kwargs) {
        ...
        var sand_paths = this.get_all_paths_from_snapshot(search_key, {'mode':'sandbox', filename_mode:'source',file_types:file_types});
    ...
    this.get_by_search_key = function() {
...
/*
TacticServerCmd = new Class({
        
    Extends: Command,
    initialize: function(server)
    {
        this.transaction_ticket = server.transaction_ticket;
        
        // copy the server
        this.server = new TacticServerStub();
        this.server.url = server.url;
        this.server.login_ticket = server.login_ticket;
        this.server.server_name = server.server_name;
        this.server.project = server.project;

        // keep the transaction ticket null
        this.server.transaction_ticket = null;
    },

    redo: function() {
        //this.server.redo(this.transaction_ticket)
        this.server.redo();
    },
    undo: function() {
        //this.server.undo(this.transaction_ticket)
        this.server.undo();
    }
...
TacticServerCmd.execute_cmd = function(class_name, panel_id, options, values)
{
    ...
    return server.execute_cmd(class_name, options, values);
}
*/
