...
//
/*
spt.dg_table_action.edit_form_cbk = function( evt, bvr )
{
...
//
// Put some expression manipulations here
//
spt.expression = {};

spt.expression.bvr = null;


spt.expression.calculate_cbk = function(evt, bvr) {
    // set the current behavior
    spt.expression.bvr = bvr;

    /*
    var expression = '{x} * {y}';
    var x = 10;
    var y = bvr.firing_data.value;
    ...
    */
    ...
    var expression = bvr.expression;
    expression = expression.replace(/@/g, 'spt.expression.');

    // Remove string formatting from end of expression string, otherwise a
    // javascript syntax error will arise in the eval() statement below ...
    var pattern = /,\s*\%\d\.\df\s*}\s*$/;
    ...
    try {
        ...
        spt.js_log.warning("Error evaluating expression: " + expression);
        spt.js_log.warning(e.message);
        spt.js_log.warning("\n");
        spt.js_log.warning(e.stack);
        return;
    }

    bvr.src_el.innerHTML = "" + result;
    bvr.src_el.setStyle("border", "solid blue 1px");
}



spt.expression.SUM = function(column) {
    var table = spt.expression.bvr.src_el.getParent(".spt_table");
    ...
    // Take into account the select row
    index += 1;

    var total = 0;
    for (var i = spt.dg_table.FIRST_ROW-1; i < tbodies.length; i++) {
        var tbody = tbodies[i];
...
spt.expression.GET = function(search_key, column) {
    var table = spt.expression.bvr.src_el.getParent(".spt_table");
    var tbodies = table.getElements(".spt_table_tbody");
    ...
    for (var i = 0; i < tbodies.length; i++) {
        var row_search_key = tbodies[i].getAttribute("spt_search_key")
        if (row_search_key == search_key) {
            tbody = tbodies[i];
        }
    ...
    var tds = tbody.getElements('.spt_table_td');
    var td = tds[index];
    td.setStyle("border", "solid 1px red");
    var value = td.getAttribute("spt_input_value");
    ...
    if (type == 'time') {
        if (value == '') {
            ...
            value = new Date();
            value.setHours(parts[0]);
            value.setMinutes(parts[1]);
            value.setSeconds(0);
        }
    }
    else if (value == '') {
